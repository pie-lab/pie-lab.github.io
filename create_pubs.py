#!/usr/bin/env python3
"""
Script to create publication folders from publications.bib

This script reads your publications.bib file and automatically creates
a folder for each publication with properly formatted index.md and cite.bib files.

Usage:
    python create_pubs.py

The script will:
1. Scan your publications.bib file
2. Check which publications already have folders
3. Create folders only for new publications
4. Generate index.md with metadata and cite.bib for each new publication
"""
...
# Base paths - automatically detects the script location
...
def create_index_md(entry):
    ...
    # Optional: set featured to true for your best publications
...
def main():
    ...
    if not BIB_FILE.exists():
        print(f"Error: publications.bib not found at {BIB_FILE}")
        print("Please create a publications.bib file in the root directory.")
        return

    if not PUB_DIR.exists():
        print(f"Creating publication directory at {PUB_DIR}")
    ...
    print(f"\nFound {len(existing_folders)} existing publication folders")
